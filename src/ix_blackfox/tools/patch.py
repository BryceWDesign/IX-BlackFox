...
class PatchFileChange:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            and self.before_sha256 != computed_before_sha256
        ):
            raise ValueError(
...
class PatchDiff:
    ...
    def __post_init__(self) -> None:
        ...
        duplicate_paths = tuple(
            sorted(path for path in set(paths) if paths.count(path) > 1)
        )
        if duplicate_paths:
            raise ValueError(
    ...
    def total_size_delta(self) -> int:
        return sum(
            len(change.after_text or "") - len(change.before_text or "")
