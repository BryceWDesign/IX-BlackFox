    ...
    def __post_init__(self) -> None:
        ...
        if self.mutation_type is PatchMutationType.REPLACE_TEXT:
            if not self.before_text:
                raise AuthoringValidationError("replace_text mutation before_text must not be empty.")
            if not self.after_text:
                raise AuthoringValidationError("replace_text mutation after_text must not be empty.")
            if self.before_text == self.after_text:
                raise AuthoringValidationError("replace_text mutation must not be a no-op.")

        if self.mutation_type is PatchMutationType.CREATE_FILE:
            if self.before_text != "":
                raise AuthoringValidationError("create_file mutation before_text must be empty.")
            if not self.after_text:
                raise AuthoringValidationError("create_file mutation after_text must not be empty.")
...
class PatchAuthoringProposal:
    ...
    def __post_init__(self) -> None:
        ...
        if self.confidence < 0.0 or self.confidence > 1.0:
            raise AuthoringValidationError("Proposal confidence must be between 0.0 and 1.0.")
        object.__setattr__(
            self,
            "assumptions",
            tuple(_normalize_text(value, label="assumption") for value in self.assumptions),
        )
        object.__setattr__(
            self,
            "risk_notes",
        ...
        object.__setattr__(
            self,
            "expected_tests",
            tuple(_normalize_text(value, label="expected_test") for value in self.expected_tests),
        )
        mutations = tuple(self.mutations)
        if not mutations:
            raise AuthoringValidationError("Patch authoring proposal requires at least one mutation.")
...
class PatchAuthoringResponseParserConfig:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            ...
            _normalize_text(self.expected_schema_version, label="expected_schema_version"),
        )
...
class PatchAuthoringResponseParser:
    ...
    def _normalize_raw_response(self, raw_response: str) -> str:
        ...
        if self.config.reject_markdown_wrapped_json and _looks_markdown_wrapped(response):
            raise _validation_error(
    ...
    def _validate_top_level_fields(self, payload: Mapping[str, Any]) -> None:
        ...
        _require_string(payload, "schema_version", max_chars=self.config.max_string_chars)
        _require_string(payload, "proposal_id", max_chars=self.config.max_string_chars)
        _require_string(payload, "objective_summary", max_chars=self.config.max_string_chars)
        _require_string(payload, "reasoning_summary", max_chars=self.config.max_string_chars)
        _require_float(payload, "confidence")
        _require_string_array(payload, "assumptions", max_chars=self.config.max_string_chars)
        _require_string_array(payload, "risk_notes", max_chars=self.config.max_string_chars)
        _require_string_array(payload, "expected_tests", max_chars=self.config.max_string_chars)
    ...
    def _parse_mutations(self, raw_mutations: list[Any]) -> tuple[PatchAuthoringMutation, ...]:
    ...
    def _validate_mutation_fields(self, mutation: Mapping[str, Any], *, index: int) -> None:
        required_fields = {
            "mutation_id",
            "mutation_type",
            "path",
        ...
        _require_string(mutation, "mutation_type", max_chars=self.config.max_string_chars)
...
def _normalize_relative_path(value: str) -> str:
    ...
    if ":" in cleaned.split("/")[0]:
        raise AuthoringValidationError(f"path must not include a drive or URI prefix: {value!r}.")
    ...
    if normalized.startswith("."):
        raise AuthoringValidationError(f"hidden root paths are not allowed in proposals: {value!r}.")
...
def _normalize_sha256(value: str) -> str:
    ...
    if len(cleaned) != 64 or any(char not in "0123456789abcdef" for char in cleaned):
        raise AuthoringValidationError("sha256 must be a 64-character lowercase hexadecimal value.")
    return cleaned
