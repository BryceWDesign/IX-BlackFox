...
class CandidateScoreBreakdown:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> Self:
        return cls(
            ...
            reasons=_coerce_text_tuple(
                payload.get("reasons", ()), field_name="reasons"
            ),
...
class RankedRepairCandidate:
    ...
    rejection_reasons: tuple[CandidateRejectionReason, ...] = field(
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _normalize_optional_identifier(
                self.policy_report_id, label="policy_report_id"
            ),
        )
        object.__setattr__(self, "metadata", dict(self.metadata))
...
class RepairCandidateSelectionReport:
    ...
    def __post_init__(self) -> None:
        ...
        if not candidates:
            raise ValueError(
                "RepairCandidateSelectionReport requires at least one candidate."
            )
    ...
    def rejected_candidates(self) -> tuple[RankedRepairCandidate, ...]:
        return tuple(
            candidate for candidate in self.ranked_candidates if candidate.rejected
        )
...
class RepairCandidateRankerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.minimum_selectable_confidence < 0.0
            or self.minimum_selectable_confidence > 1.0
        ):
            raise ValueError(
                "minimum_selectable_confidence must be between 0.0 and 1.0."
            )
        if self.maximum_path_risk_score_for_selection < 0.0:
            raise ValueError(
                "maximum_path_risk_score_for_selection must be zero or greater."
            )
...
class RepairCandidateRanker:
    ...
    config: RepairCandidateRankerConfig = field(
        ...
        *,
        ...
        for candidate in candidate_tuple:
            ...
            if (
                ...
                and policy_report.proposal_digest != candidate.proposal_digest
            ):
                rejection_reasons.append(
            ...
            if (
                ...
                and proposal.confidence < self.config.minimum_selectable_confidence
            ):
                rejection_reasons.append(CandidateRejectionReason.LOW_CONFIDENCE)

            if (
                score.path_risk_score
                > self.config.maximum_path_risk_score_for_selection
            ):
                rejection_reasons.append(CandidateRejectionReason.PATH_RISK_TOO_HIGH)

            if (
                abs(_patch_size_delta(candidate))
                > self.config.maximum_total_size_delta_for_selection
            ):
            ...
            scored.append(
                RankedRepairCandidate(
                    ...
                    rejection_reasons=tuple(
                        _dedupe_rejection_reasons(rejection_reasons)
                    ),
                    ...
                    policy_report_id=None
                    if policy_report is None
                    else policy_report.report_id,
                    metadata={
        ...
        for index, ranked_candidate in enumerate(ranked):
            if (
                index == 0
                and ranked_candidate.disposition is CandidateDisposition.AVAILABLE
            ):
            ...
            if ranked_candidate.disposition is CandidateDisposition.AVAILABLE:
                final_ranked.append(
                    _replace_ranked_disposition(
                        ranked_candidate,
                        disposition=CandidateDisposition.REJECTED,
                        rejection_reasons=(CandidateRejectionReason.NOT_TOP_RANKED,),
                    )
                )
                continue
    ...
    def _score_candidate(
        ...
        *,
        ...
        path_risk_score = self._path_risk_score(
            candidate.changed_paths, reasons=reasons
        )
                ...
                AuthoringFinding(
                    ...
                    metadata={
                        ...
                        "review_required_count": sum(
                            ...
                            for candidate in ranked_candidates
                            if candidate.disposition
                            is CandidateDisposition.REQUIRES_REVIEW
                        ),
                    },
                )
            )
...
def _patch_size_delta(candidate: CompiledPatchCandidate) -> int:
    return candidate.patch_diff.total_size_delta
