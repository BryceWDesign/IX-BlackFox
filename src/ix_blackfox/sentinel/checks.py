            ...
            if decision == "block" and executed:
                issues.append(
                    SentinelIssue(
                        ...
                        summary=(
                            "Governance reported a blocked action as executed."
                        ),
            ...
            if executed and approval_required and not approval_satisfied:
                issues.append(
                    SentinelIssue(
                        ...
                        severity=SentinelSeverity.ERROR,
                        summary=(
                            "Runtime execution proceeded despite an unsatisfied approval gate."
                        ),
                        source=self.check_name,
                        details=(
...
class TaskStateTraceContradictionCheck(SentinelCheck):
    ...
    def evaluate(self, context: SentinelContext) -> tuple[SentinelIssue, ...]:
        ...
        normalized_messages = tuple(message.lower() for message in trace_messages if message)
        ...
        if task_state == "completed" and _contains_failure_signal(normalized_messages):
            issues.append(
                SentinelIssue(
                    ...
                    summary=(
                        "Task state is completed, but trace records contain failure signals."
                    ),
                    source=self.check_name,
                    details=_state_trace_details(task_state=task_state, messages=trace_messages),
                    data={"task_state": task_state},
                )
            )

        if task_state == "failed" and _contains_completion_signal(normalized_messages):
            issues.append(
                SentinelIssue(
                    code="sentinel.task_state_contradiction",
                    severity=SentinelSeverity.WARNING,
                    summary=(
                        "Task state is failed, but trace records contain completion signals."
                    ),
                    source=self.check_name,
                    details=_state_trace_details(task_state=task_state, messages=trace_messages),
                    data={"task_state": task_state},
...
def _contains_failure_signal(messages: Iterable[str]) -> bool:
    ...
    return any(any(indicator in message for indicator in indicators) for message in messages)
...
def _state_trace_details(*, task_state: str, messages: tuple[str, ...]) -> str:
    ...
    return (
        f"Contradictory trace evidence was observed for task state '{task_state}': {joined}"
    )
