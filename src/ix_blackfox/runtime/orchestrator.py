...
class RuntimeRunReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.safeguard_assessment is not None:
            safeguard_assessment = _safeguard_assessment_to_dict(
                self.safeguard_assessment
            )
...
class BlackFoxRuntime:
    ...
    def execute_task(self, task: TaskRecord) -> RuntimeRunReport:
        ...
        elif safeguard_outcome.succeeded and safeguard_outcome.assessment is not None:
            assessment = safeguard_outcome.assessment
            self._append_trace(
                correlation_id=task.request.task_id,
                stage="safeguard",
                message=(
                    ...
                    f"disposition '{assessment.advisory_disposition.value}'."
                ),
                level="info",
                source="brain.safeguard",
            )
        ...
        if governance_preflight.blocked:
            ...
            reasoning_outcome = self._run_reasoning_lane_if_applicable(
                task=failed_task,
                route=route,
                sentinel_report=sentinel_report,
                verification=verification,
            ...
            return self._finalize_run(
                ...
                vision_outcome=vision_outcome,
                policy_advisory=policy_outcome.assessment
        ...
        pack_context = PackContext(
            ...
            brain=_pack_brain_context(
                plan=primary_brain_plan, outcome=primary_brain_outcome
            ),
            metadata={
                ...
                else _vision_outcome_to_dict(vision_outcome),
                "policy_advisory": None
    ...
    def _run_policy_reasoning_lane(
        ...
        *,
        ...
        plan = self._policy_reasoning_runtime.plan(
            ...
            if vision_outcome is None
            else vision_outcome.observation_text,
        )
        ...
        if outcome.skipped:
        ...
        elif outcome.succeeded and outcome.assessment is not None:
            ...
            self._append_trace(
                ...
                message=(
                    ...
                    f"'{assessment.advisory_disposition.value}'."
                ),
                level="info",
                source="brain.policy",
            )
    ...
    def _run_reasoning_lane_if_applicable(
        self,
        *,
        task: TaskRecord,
        route: RoutingDecision,
        ...
        escalation_decision: BrainEscalationDecision,
        brain_receipt_ledger: BrainInvocationReceiptLedger,
    ) -> EscalatedReasoningOutcome | None:
        if not escalation_decision.should_escalate:
            return None

        sentinel_issue_codes = tuple(issue.code for issue in sentinel_report.issues)
        plan = self._reasoning_runtime.plan(
            task=task,
            route=route,
            pack_name=route.capability_name,
            escalation_score=escalation_decision.score,
            trigger_codes=escalation_decision.trigger_codes(),
            verification_status=verification.status.value,
            sentinel_issue_codes=sentinel_issue_codes,
            prior_failure_message=task.error,
        )
        outcome = self._reasoning_runtime.invoke(
            plan=plan,
            providers=self._brain_providers,
            receipt_ledger=brain_receipt_ledger,
        )
        ...
        if outcome.skipped:
            self._append_trace(
                ...
                stage="reasoning",
                message=(
                    f"Reasoning brain '{plan.manifest.brain_name}' was planned "
                    f"but not invoked: {outcome.failure_message}"
                ),
                level="warning",
                source="brain.reasoning",
            )
        elif outcome.succeeded:
            self._append_trace(
                correlation_id=task.request.task_id,
                stage="reasoning",
                message=(
                    f"Reasoning brain '{plan.manifest.brain_name}' completed through provider "
                    f"'{outcome.provider_name}' with summary '{outcome.summary or 'n/a'}'."
                ),
                level="info",
                source="brain.reasoning",
            )
            ...
            self._append_trace(
                ...
                stage="reasoning",
                message=(
                    f"Reasoning brain '{plan.manifest.brain_name}' invocation failed: "
                    f"{outcome.failure_message}"
                ),
                level="warning",
                source="brain.reasoning",
    ...
    def _finalize_run(
        ...
        *,
        ...
        report = RuntimeRunReport(
            ...
            if reasoning_outcome is None
            else _reasoning_outcome_to_dict(reasoning_outcome),
            vision_outcome=None
            if vision_outcome is None
            else _vision_outcome_to_dict(vision_outcome),
            policy_advisory=policy_advisory,
    ...
    def _register_default_manifests(self) -> None:
        ...
        self._switchboard.register(
            CapabilityRoute(
                ...
                supported_kinds=(*programming.supported_kinds, TaskKind.OPERATIONS),
                labels=(
                    *programming.labels,
                    "operations",
                    "network",
...
def _reasoning_outcome_to_dict(outcome: EscalatedReasoningOutcome) -> dict[str, Any]:
    status = None
    if outcome.result is not None:
    ...
    return {
        ...
        "escalation_score": outcome.plan.escalation_score,
        "trigger_codes": list(outcome.plan.trigger_codes),
        "invoked": outcome.invoked,
...
def _coerce_string_sequence(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        return [] if not cleaned_text else [cleaned_text]
